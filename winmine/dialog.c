    ...
    case WM_COMMAND:
        ...
        case IDOK:
            GetDlgItemTextW( hDlg, IDC_EDITNAME,
                             ...
                             _countof( p_board->best_name[p_board->difficulty] ));
            EndDialog( hDlg, 0 );
            return TRUE;
...
INT_PTR CALLBACK TimesDlgProc( HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam )
{
    ...
    case WM_COMMAND:
        switch( LOWORD( wParam ) ) {
        case IDC_RESET:
            LoadStringW( NULL, IDC_CONFIRMTITLE, confirm_title, _countof(confirm_title));
            LoadStringW( NULL, IDC_CONFIRMTEXT, confirm_text, _countof(confirm_text));
